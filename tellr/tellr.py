...
def main():
    ...
    parser.add_argument('-TE_ref', '--TE_ref', help='fasta fil with elements to be detected', required=True, type=pathlib.Path)
    ...
    for contig in bam_header["SQ"]:
        chrs.append(contig["SN"])
        chr_length[ contig["SN"] ]=contig["LN"]
    ...
    calls = assembleandcall.main(bam_name, repeat_fasta, sample, clustered[1], candidates[2]) 
